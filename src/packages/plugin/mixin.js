...
export default {
  computed: {
     ...
     * 获取字典请求数据
     */
    $requestAssignDict(val) {
      return new Promise((resolve) => {
        this.$getDicts(val).then(({ data: dicts }) => {
          dicts.forEach((r) => {
            r[this._getDictValues[0]] = r[[this._getDictKeys[0]]];
            r[this._getDictValues[1]] = r[[this._getDictKeys[1]]];
          });
          resolve(dicts);
        });
      });
  ...
  methods: {
    ...
    $minioDownFiles(file, item) {
      ...
      download(`${url}/${file.fileId}`, {}, file.name, this.$request);
    },
  },
};
