<template>
  <el-popover ref="popover" popper-class="kt-select-tree__popover" placement="bottom-start" trigger="click" :disabled="disabled">
    <!-- 显示content -->
    <div id="treeContent" ref="treeContent">
      <el-input class="kt-select-tree__input" v-if="filterable" placeholder="请输入关键字" v-model="searchValue"></el-input>
      <el-tree
        ref="tree"
  ...
  methods: {
    ...
    onTreeFilter(value, data) {
      ...
      return data[this.dynaDefProps.filter]
        ? data[this.dynaDefProps.label].indexOf(value) !== -1 || data[this.dynaDefProps.filter].indexOf(value.toUpperCase()) !== -1
    ...
    isArray(data) {
      return (
        data[this.dynaDefProps.children] && Array.isArray(data[this.dynaDefProps.children]) && data[this.dynaDefProps.children].length > 0
      );
    },
  },
  mounted() {
